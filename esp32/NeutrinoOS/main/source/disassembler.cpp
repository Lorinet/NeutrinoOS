...
Array<instruction>* disassembler::disassembleCode(byte* bc, int size)
{
	...
	while (cntr < size)
	{
		...
		else if (op == opcode::OP_LDGLB || op == opcode::OP_STGLB || op == opcode::OP_LDLOCB || op == opcode::OP_STLOCB || op == opcode::OP_INTR || op == opcode::OP_LDB || op == opcode::OP_LDIB || op == opcode::OP_TOPB || op == opcode::OP_SWSCOPB)
		{
			length = 1;
		}
