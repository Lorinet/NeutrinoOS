...
Array<instruction>* disassembler::disassembleCode(byte* bc, int size)
{
	...
	while (cntr < size)
	{
		...
		else if (op == opcode::ADD || op == opcode::SUB || op == opcode::MUL || op == opcode::DIV || op == opcode::VAC || op == opcode::VAD || op == opcode::VADE || op == opcode::VAI || op == opcode::VAR || op == opcode::VAL || op == opcode::SWAP || op == opcode::VAP || op == opcode::VAS || op == opcode::VPF || op == opcode::NOP || op == opcode::RET || op == opcode::HALT || op == opcode::BREAK || op == opcode::SPOP || op == opcode::SCMP)
		{
			length = 0;
		}
		else if (op == opcode::TOSTR || op == opcode::MOV || op == opcode::CONCAT || op == opcode::PARSE || op == opcode::SIZ || op == opcode::APPEND|| op == opcode::INC || op == opcode::DEC || op == opcode::IMUL || op == opcode::IDIV || op == opcode::CMPI || op == opcode::CMP || op == opcode::AND || op == opcode::OR || op == opcode::XOR || op == opcode::SHL || op == opcode::SHR || op == opcode::EXTCALL || op == opcode::MOVL /*|| op == opcode::VAD || op == opcode::VPF || op == opcode::VAR || op == opcode::VAP || op == opcode::VAL*/)
		{
			length = 8;
		}
		...
		else if (op == opcode::SPLIT || op == opcode::INDEX || op == opcode::INSERT || op == opcode::PUSHBLK || op == opcode::EXTMOVL)
		{
			length = 12;
		}
